...
def solution_1a(points, stc, stic, ec,  result_of_cost, connected_edges, speed, capacity, b, alpha, beta, source, destination, commodities):
  print(f"\n----------------------in solution 1a ZUB----------------\n")
  ...
  if lp.status == -1:  # Infeasible
    ...
    print("\nVariable Bounds:")
    for (i, j) in arcs:
        value = pulp.lpSum(flow[k, i, j].varValue for k in commodities if flow[k, i, j].varValue is not None)
