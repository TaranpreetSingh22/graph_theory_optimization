...
def yij_graph(input_points):
    ...
    y_ij = df["yij"].iloc[-1]  # Assuming dictionary is stored in a single row
    ...
    nx.draw(G, pos=nodes_coordinates, with_labels=True, node_size=500, node_color="lightblue", edge_color="gray")
    plt.show()
...
def fij_graph(input_points, commodities):
    ...
    y_ij = ast.literal_eval(df["yij"].iloc[-1])  # (i, j) : binary_val
    f_ij = ast.literal_eval(df["fij"].iloc[-1])  # (k, i, j) : flow_value
    ...
    edge_widths = [data["weight"] * 5 for _, _, data in edges]  # Scale widths for visibility
    ...
    for k in unique_commodities:
        ...
        for (k_flow, i, j), flow in f_ij.items():
            if k_flow == k and flow > 0:  # Only consider edges for the current commodity
                ...
                edge_widths.append(flow * 5)  # Scale widths for visibility
